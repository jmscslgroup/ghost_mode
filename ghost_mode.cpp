...
void ghost_mode_step(void)
{
  ...
  if (rtmIsMajorTimeStep(ghost_mode_M)) {
    ...
    ghost_mode_B.ProportionalGain = ghost_mode_P.PIDJMSTransport_P *
      rtb_IntegralGain;

    // Gain: '<S33>/Derivative Gain'
    ghost_mode_B.DerivativeGain = ghost_mode_P.PIDJMSTransport_D *
      rtb_IntegralGain;
  }
  ...
  ghost_mode_B.FilterCoefficient = (ghost_mode_B.DerivativeGain -
    ghost_mode_X.Filter_CSTATE) * ghost_mode_P.PIDJMSTransport_N;
  ...
  if (rtb_SignPreSat > ghost_mode_P.PIDJMSTransport_UpperSaturation) {
    rtb_ZeroGain = ghost_mode_P.PIDJMSTransport_UpperSaturation;
  } else if (rtb_SignPreSat < ghost_mode_P.PIDJMSTransport_LowerSaturation) {
    rtb_ZeroGain = ghost_mode_P.PIDJMSTransport_LowerSaturation;
  } else {
  ...
  if (rtb_SignPreSat > ghost_mode_P.PIDJMSTransport_UpperSaturation) {
    rtb_SignPreSat -= ghost_mode_P.PIDJMSTransport_UpperSaturation;
  } else if (rtb_SignPreSat >= ghost_mode_P.PIDJMSTransport_LowerSaturation) {
    rtb_SignPreSat = 0.0;
  } else {
    rtb_SignPreSat -= ghost_mode_P.PIDJMSTransport_LowerSaturation;
  }

  // End of DeadZone: '<S32>/DeadZone'
  if (rtmIsMajorTimeStep(ghost_mode_M)) {
    // Gain: '<S36>/Integral Gain'
    rtb_IntegralGain *= ghost_mode_P.PIDJMSTransport_I;
...
void ghost_mode_initialize(void)
{
...

  {
    ...
    ghost_mode_X.Integrator_CSTATE =
      ghost_mode_P.PIDJMSTransport_InitialCondit_d;

    // InitializeConditions for Integrator: '<S34>/Filter'
    ghost_mode_X.Filter_CSTATE = ghost_mode_P.PIDJMSTransport_InitialConditio;
